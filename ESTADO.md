...
| 2026-08-09 | Fallback no reintentaba en el bug de tool_call_id | El bug de ADR-024/026 no es un `429`, así que la condición de reintento (`grep -qE '429\|rate.limit\|...'`) no lo detectaba como reintentable y cortaba la cadena de modelos en el primer golpe, en vez de probar el siguiente. Se reprodujo en issue #6 (tarea trivial: un solo rename de archivo) — los tres agentes chocaron con el bug en `oc/big-pickle` y ninguno probó `oc/deepseek-v4-flash-free` ni Codex. | Se agregó `Duplicate value for .tool_call_id.` al patrón de reintento en `scripts/ejecutar-issue.sh` (dos lugares: planificador y agentes). Confirmado que ahora sí avanza a la siguiente ruta. |
| 2026-08-09 | OmniRoute: límite de conexiones subido 20→40 | Con 20 seguía saturándose (\"at connection limit\") incluso para el planificador solo, sin exploración. El tráfico de fondo de openclaw-gateway más el uso normal de la flota lo agotan igual. | Subido a 40. Sigue siendo un recurso local sin costo — conviene revisar si hay una conexión realmente colgada (no solo alta demanda) si esto se repite otra vez. |
| 2026-08-09 | Issue #6: los agentes completaron trabajo periférico pero no el objetivo real | Los tres agentes (backend, tests, docs) commitearon con éxito — pero ninguno borró/corrigió el archivo `.ml` que era el objetivo del issue (G-05). Backend agregó una verificación, tests agregó un test sobre el `.md` gemelo, docs no actualizó el backlog. El PR que hubiera abierto el script automático habría declarado el issue resuelto sin estarlo. | Se detectó revisando el diff real antes de abrir el PR (no confiar en que "3 commits exitosos" == "objetivo cumplido"). Se agregó el fix real a mano (borrar el `.ml`, actualizar el backlog) antes de abrir [PR #20](https://github.com/marksato13/ninjasec-platform/pull/20). **Lección: revisar el diff real de `integra/issue-N` contra lo que pedía el issue antes de confiar en que "sin conflictos y CI verde" significa "issue resuelto".** |
| 2026-08-08 | Issue #4, cerrado con éxito — PR #5 | Con Codex forzado (`CODEX_*_MODEL=cx/...`) los tres agentes completaron su trabajo real por primera vez. Al integrar, `docs/issue-4` chocó con un conflicto de merge **real** (no el bug de OmniRoute) contra `test/issue-4` en `23-MODULO-TOPOLOGIA.md` — `integrar.sh` lo reportó y abortó el merge automáticamente (comportamiento correcto: "el Revisor debe resolverlo, si es ambiguo escalar"). Se resolvió a mano: se confirmó que solo el `.md` estaba en conflicto (el código real de `test/issue-4` — `router.py`, `topology_service.py`, `test_topology.py` — se había mergeado limpio antes), se tomó la versión de `docs/issue-4` (más completa) y se le agregó la evidencia de tests de `test/issue-4` sin perder contenido de ninguna rama. | **[PR #5](https://github.com/marksato13/ninjasec-platform/pull/5) abierto en borrador**, CI corriendo (secretos: OK). Notificado por Telegram. Pendiente: que el usuario revise y apruebe con `/a 5` + `/c CODIGO` — no se automatizó la resolución de conflictos de integración; sigue siendo un paso manual cuando `integra/issue-*` choca con otra rama del mismo issue. |
| 2026-08-08 | Corrección: cuota de Codex y límite de conexiones se confundían | La cuota de Codex/ChatGPT volvió a estar disponible el 2026-08-08 ~21:56, varios días antes del 12/08 anunciado. Muchos `429` vistos esa tarde en `cx/*` eran en realidad el límite de conexiones concurrentes de OmniRoute saturado (ver fila siguiente), no la cuota real — ambos devuelven HTTP 429 y son indistinguibles sin mirar `docker logs omniroute`. Ver ADR-026. | Antes de asumir "cuota agotada", probar `codex exec "responde solo: ok"` directo y revisar `docker logs omniroute` por `"at connection limit"`. |
| 2026-08-08 | OmniRoute: límite de conexiones subido otra vez, 8→20 | Con Codex ya disponible, `cx/gpt-5.6-sol` seguía dando 429 — resultó ser el límite de conexiones (8, subido antes de 4) saturado por el tráfico de fondo de `openclaw-gateway` + Ollama + las pruebas. | Subido a 20 en `infra/docker-compose.yml`. Es un recurso local sin costo, no hay razón para ser conservador. |
